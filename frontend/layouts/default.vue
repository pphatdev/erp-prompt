  ...
  <div class="min-h-screen flex bg-(--bg-layout) text-(--text-body)">
    ...
    >
      ...
      <button
        ...
        @mouseleave="onSidebarLeave"
      >
        <i class="ti text-[12px] transition-transform" :class="sidebarCollapsed ? 'ti-chevron-right' : 'ti-chevron-left'" />
      </button>

      <!-- Brand -->
      <div class="h-16 border-b border-(--border-color) flex items-center" :class="isCompact ? 'px-3 justify-center' : 'px-5'">
        <NuxtLink to="/dashboard" class="flex items-center gap-3 min-w-0" @click="closeMobileSidebar">
          <span class="w-9 h-9 rounded-lg bg-linear-to-tr from-(--color-primary) to-(--color-info) flex items-center justify-center text-white shadow-(--shadow-sm) shrink-0">
          ...
          <span v-show="!isCompact" class="overflow-hidden">
            ...
            <span class="block text-[10px] uppercase tracking-widest font-mono text-(--text-muted)">Enterprise Suite</span>
          </span>
        </NuxtLink>
      </div>
      ...
      <nav class="flex-1 px-3 py-4 space-y-5 overflow-y-auto custom-scrollbar">
        ...
        <div v-for="group in visibleNavGroups" :key="group.id" class="space-y-1">
          ...
          <template v-for="item in group.items" :key="item.label">
            ...
            <NuxtLink
              v-if="!item.children && item.operational"
              :to="item.route!"
              class="nav-link"
            ...
            >
              ...
              <Badge v-show="!isCompact && item.badge" :variant="item.badgeVariant || 'success'">{{ item.badge }}</Badge>
            </NuxtLink>
            ...
            <button
              ...
              type="button"
              class="nav-link nav-link-disabled w-full"
              @click="comingSoon(item.label)"
            >
              <span class="nav-icon"><i :class="['ti', item.icon]" /></span>
              <span v-show="!isCompact" class="truncate flex-1 text-left">{{ item.label }}</span>
              <span v-show="!isCompact" class="text-xxs font-mono uppercase tracking-wider text-(--text-muted)">soon</span>
            </button>
    ...
    <!-- ============================ Main viewport ============================ -->
    <div class="flex-1 flex flex-col min-w-0 transition-all duration-300" :class="sidebarCollapsed ? 'md:pl-[70px]' : 'md:pl-[260px]'">
      <!-- ----------------------- Topbar §4.1 ----------------------- -->
      <header class="sticky top-0 z-20 h-16 bg-(--header-bg) backdrop-blur-xl border-b border-(--border-color) flex items-center justify-between px-4 sm:px-6">
        <div class="flex items-center gap-4 min-w-0">
          <button
            type="button"
            class="md:hidden p-2 rounded-lg hover:bg-(--bg-muted) text-(--text-body)"
        ...
        <div class="hidden lg:flex items-center w-80 relative">
          ...
          <input
            type="search"
            placeholder="Search transactions, files, roles..."
            class="form-control pl-9 pr-12 py-1.5 text-xs"
          />
          <span class="absolute right-2.5 top-1/2 -translate-y-1/2 text-xxs font-mono font-semibold px-1.5 py-0.5 rounded border border-(--border-color) bg-(--bg-muted) text-(--text-muted)">⌘K</span>
        </div>
        ...
        <div class="flex items-center gap-1.5">
          ...
          <div class="relative">
            <button class="topbar-btn" type="button" @click.stop="toggle('notif')">
              ...
              <span class="absolute top-1 right-1 min-w-[14px] h-[14px] px-1 rounded-full bg-(--color-danger) text-[9px] font-bold text-white flex items-center justify-center font-mono">7</span>
            </button>
            <transition name="popover">
              <div v-if="open.notif" class="popover w-80 p-0" @click.stop>
                ...
                <ul class="max-h-72 overflow-y-auto divide-y divide-(--border-color)">
                  <li v-for="n in notifications" :key="n.id" class="px-4 py-3 hover:bg-(--bg-muted) flex gap-3">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0" :class="iconWrapMap[n.tone]">
                      <i :class="['ti', n.icon]" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <p class="text-xs font-semibold text-(--text-heading) truncate">{{ n.title }}</p>
                      <p class="text-xxs text-(--text-muted) truncate">{{ n.detail }} · {{ n.time }}</p>
                    </div>
                  </li>
                </ul>
              </div>
          ...
          <div v-if="!hasSubdomain" class="relative">
            <button class="topbar-btn px-3 gap-2" type="button" @click.stop="toggle('tenant')">
              <span class="w-2 h-2 rounded-full" :style="{ background: `rgb(${tenantStore.currentTenant?.primaryColor || '59 130 246'})` }" />
              <span class="text-xs font-semibold text-(--text-heading) truncate max-w-[120px]">{{ tenantStore.activeName }}</span>
            ...
            <transition name="popover">
              <div v-if="open.tenant" class="popover w-60 p-1.5" @click.stop>
                <div class="px-3 py-2 border-b border-(--border-color) mb-1">
                  <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Select tenant</span>
                </div>
                <button
                ...
                >
                  <span class="w-2 h-2 rounded-full" :style="{ background: `rgb(${t.primaryColor})` }" />
          ...
          <div class="relative">
            <button class="topbar-btn p-1" type="button" @click.stop="toggle('profile')">
              <div class="relative">
                <div class="w-8 h-8 rounded-full bg-(--color-primary-subtle) text-(--color-primary) flex items-center justify-center font-semibold text-xs">
                  {{ initials }}
                </div>
                <span class="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-(--color-success) border-2 border-(--bg-card)" />
              </div>
            </button>
            <transition name="popover">
              <div v-if="open.profile" class="popover w-60 p-1.5" @click.stop>
                <div class="px-3 py-2 border-b border-(--border-color) mb-1">
                  <p class="text-xs font-semibold text-(--text-heading) truncate">{{ authStore.user?.name || 'Guest User' }}</p>
                  <p class="text-xxs text-(--text-muted) truncate">{{ authStore.user?.email || 'no session' }}</p>
                </div>
                <button v-for="entry in profileMenu" :key="entry.label" type="button" class="profile-item">
                  <i :class="['ti', entry.icon]" />
                  <span>{{ entry.label }}</span>
                </button>
                <hr class="my-1 border-(--border-color)" />
                <button type="button" class="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-(--color-danger) hover:bg-(--color-danger-subtle)" @click="handleLogout">
      ...
      <footer class="px-6 py-4 border-t border-(--border-color) bg-(--footer-bg) flex flex-col sm:flex-row justify-between items-center gap-2 text-xxs text-(--text-muted)">
        <div>
          © {{ new Date().getFullYear() }} Smart ERP · Tenant <span class="font-mono text-(--text-body)">@{{ tenantStore.activeHandle }}</span>
        </div>
...
.nav-link:hover { background: var(--bg-muted); color: var(--text-heading); }
.nav-link-active {
...
.profile-item:hover { background: var(--bg-muted); color: var(--text-heading); }
...
.popover-enter-active, .popover-leave-active { transition: opacity 0.15s ease, transform 0.15s ease; }
