        ...
        <div class="confirm-card" :class="`confirm-card--${active.color}`">
          ...
          <div class="confirm-actions">
            ...
            <button
              ...
              type="button"
              class="confirm-btn"
              :class="`confirm-btn--${active.color}`"
              @click="confirm"
            >
              {{ active.confirmLabel }}
            </button>
...
.confirm-card--primary   { --accent: rgb(var(--color-primary-rgb)   / 0.18); }
.confirm-card--success   { --accent: rgb(var(--color-success-rgb)   / 0.18); }
.confirm-card--warning   { --accent: rgb(var(--color-warning-rgb)   / 0.20); }
.confirm-card--danger    { --accent: rgb(var(--color-danger-rgb)    / 0.18); }
.confirm-card--info      { --accent: rgb(var(--color-info-rgb)      / 0.18); }
.confirm-card--secondary { --accent: rgb(var(--color-secondary-rgb) / 0.18); }
...
.confirm-close:hover { background: var(--bg-muted); color: var(--text-heading); }
...
.confirm-title {
  font-size: 1.0625rem;          /* 17px — friendly heading, not page-title aggressive */
  font-weight: 600;
...
.confirm-btn:hover { background: rgb(var(--color-primary-rgb) / 0.9); }
.confirm-btn:focus-visible {
  outline: none;
  box-shadow: 0 0 0 3px rgb(var(--color-primary-rgb) / 0.3), 0 6px 16px -4px rgb(var(--color-primary-rgb) / 0.4);
}

.confirm-btn--danger    { background: var(--color-danger);  box-shadow: 0 6px 16px -4px rgb(var(--color-danger-rgb)  / 0.4); }
.confirm-btn--danger:hover  { background: rgb(var(--color-danger-rgb)  / 0.9); }
.confirm-btn--danger:focus-visible  { box-shadow: 0 0 0 3px rgb(var(--color-danger-rgb)  / 0.3), 0 6px 16px -4px rgb(var(--color-danger-rgb)  / 0.4); }

.confirm-btn--warning   { background: var(--color-warning); color: #1f1300; box-shadow: 0 6px 16px -4px rgb(var(--color-warning-rgb) / 0.4); }
.confirm-btn--warning:hover { background: rgb(var(--color-warning-rgb) / 0.9); }
.confirm-btn--warning:focus-visible { box-shadow: 0 0 0 3px rgb(var(--color-warning-rgb) / 0.3), 0 6px 16px -4px rgb(var(--color-warning-rgb) / 0.4); }

.confirm-btn--success   { background: var(--color-success); box-shadow: 0 6px 16px -4px rgb(var(--color-success-rgb) / 0.4); }
.confirm-btn--success:hover { background: rgb(var(--color-success-rgb) / 0.9); }
.confirm-btn--success:focus-visible { box-shadow: 0 0 0 3px rgb(var(--color-success-rgb) / 0.3), 0 6px 16px -4px rgb(var(--color-success-rgb) / 0.4); }

.confirm-btn--info      { background: var(--color-info); box-shadow: 0 6px 16px -4px rgb(var(--color-info-rgb) / 0.4); }
.confirm-btn--info:hover { background: rgb(var(--color-info-rgb) / 0.9); }

.confirm-btn--secondary { background: var(--color-secondary); box-shadow: 0 6px 16px -4px rgb(var(--color-secondary-rgb) / 0.4); }
.confirm-btn--secondary:hover { background: rgb(var(--color-secondary-rgb) / 0.9); }
...
.confirm-enter-from .confirm-card { transform: translateY(12px) scale(0.96); opacity: 0; }
.confirm-leave-to   .confirm-card { transform: translateY(8px) scale(0.98);  opacity: 0; }

@media (prefers-reduced-motion: reduce) {
  .confirm-halo { animation: none; }
  .confirm-enter-active .confirm-card,
  .confirm-leave-active .confirm-card { transition: opacity 0.15s ease; transform: none; }
  .confirm-enter-from .confirm-card,
  .confirm-leave-to   .confirm-card   { transform: none; }
}
</style>
