<template>
  ...
  <div class="relative overflow-hidden p-5 rounded-xl border border-(--border-color) bg-(--bg-card) shadow-(--shadow-sm)">
    <span
      v-for="(meteor, idx) in count"
      :key="idx"
      class="absolute h-px w-[120px] bg-linear-to-r animate-meteor pointer-events-none"
      :class="streakClass"
