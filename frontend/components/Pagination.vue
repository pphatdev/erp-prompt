<template>
  <nav
    v-if="totalPages > 1 || showMeta"
    class="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 text-xs"
    aria-label="Pagination"
  >
    <div class="text-(--text-muted) font-mono">
      <span v-if="total > 0">
        Showing <span class="text-(--text-heading) font-semibold">{{ rangeStart }}</span>–<span class="text-(--text-heading) font-semibold">{{ rangeEnd }}</span>
    ...
    <div class="flex items-center gap-1">
      ...
      <button
        ...
        :key="p"
        type="button"
        class="page-btn min-w-[28px]"
        :class="p === page ? 'page-btn-active' : ''"
      ...
      <select
        ...
        :value="limit"
        class="form-control form-control-sm ml-2 w-[88px]"
