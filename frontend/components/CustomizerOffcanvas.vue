      ...
      >
        <header class="h-14 px-5 border-b border-(--border-color) flex items-center justify-between">
          <div>
            ...
            <p class="text-xxs text-(--text-muted) font-mono uppercase tracking-widest">Realtime theme tuning</p>
          </div>
          <button
            class="w-8 h-8 rounded-full hover:bg-(--bg-muted) text-(--text-body)"
            @click="$emit('update:modelValue', false)"
          >
            <i class="ti ti-x" />
          </button>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-7">
          ...
          <section>
            ...
            <div class="grid grid-cols-3 gap-2">
              <button
                ...
                :key="skin.key"
                type="button"
                class="rounded-lg border p-2 text-xxs font-semibold transition-all"
                :class="activeSkin === skin.key
          ...
          <!-- §6.2 — Color Scheme -->
          <section>
            <h5 class="text-xxs font-bold uppercase tracking-widest text-(--text-muted) mb-3">Color scheme</h5>
            <div class="grid grid-cols-3 gap-2">
              <button
                v-for="mode in modes"
                :key="mode.key"
          ...
          <!-- §6.3 — Accent tuning -->
          <section>
            <h5 class="text-xxs font-bold uppercase tracking-widest text-(--text-muted) mb-3">Primary accent</h5>
...
const reset = () => {
  setMode('light')
  setAccent('59 130 246')
  activeSkin.value = 'paces'
}

onMounted(() => {
  const savedAccent = localStorage.getItem('accent')
  if (savedAccent) activeAccent.value = savedAccent
