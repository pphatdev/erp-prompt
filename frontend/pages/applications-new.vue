    ...
    <div class="space-y-6">
      ...
      <header class="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
        ...
        <ol class="flex items-center gap-1 sm:gap-2 self-start xl:self-end">
          <li v-for="(s, i) in STEPS" :key="s.id" class="flex items-center gap-1 sm:gap-2">
            ...
            <span v-if="i < STEPS.length - 1" class="step-bar" :class="isComplete(i) ? 'step-bar-done' : ''" />
          </li>
        </ol>
      </header>
      ...
      <section v-if="activeStep === 0" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-5">
          <div
            class="glass-card rounded-2xl p-8 transition-all"
          ...
          >
            <div class="text-center flex flex-col items-center gap-3 py-6">
              <span class="w-14 h-14 rounded-2xl bg-(--color-primary-subtle) text-(--color-primary) inline-flex items-center justify-center">
              ...
              <div class="flex items-center gap-2 mt-2">
                ...
                >
                  <i :class="['ti', uploadState === 'uploading' ? 'ti-loader animate-spin' : 'ti-file-upload']" />
                  {{ uploadState === 'uploading' ? 'Uploading...' : 'Browse file' }}
                </button>
                <button
                  v-if="resumeFile || uploadState === 'failed'"
                  type="button"
                ...
                <button
                  v-if="uploadState === 'failed' && resumeFile"
                  type="button"
              ...
              <div v-if="resumeFile" class="mt-2 flex flex-col items-center gap-1">
                ...
                <p v-if="uploadState === 'uploading'" class="text-xxs text-(--color-info) inline-flex items-center gap-1">
                  <span class="w-3 h-3 rounded-full border-2 border-(--color-info)/30 border-t-(--color-info) animate-spin" />
                  Uploading to tenant storage...
                </p>
                <p v-else-if="uploadState === 'success'" class="text-xxs text-(--color-success) inline-flex items-center gap-1">
                  <i class="ti ti-circle-check" /> Uploaded · stored as <span class="font-mono">{{ uploadedPath }}</span>
                </p>
                <p v-else-if="uploadState === 'failed'" class="text-xxs text-(--color-danger) inline-flex items-center gap-1">
          ...
          <div class="glass-card rounded-2xl p-5">
            <h4 class="text-xs font-bold uppercase tracking-widest text-(--text-muted) mb-3">Skip the upload</h4>
        ...
        <aside class="space-y-4">
          <div class="glass-card rounded-2xl p-5">
            <h4 class="text-xs font-bold uppercase tracking-widest text-(--text-muted) mb-3">Target vacancy</h4>
          ...
          <div class="glass-card rounded-2xl p-5 space-y-3">
            ...
            <ul class="text-xs text-(--text-body) space-y-2">
              <li class="flex gap-2"><i class="ti ti-info-circle text-(--color-info) mt-0.5" /> PDF, DOC, DOCX accepted — up to 10 MB.</li>
              <li class="flex gap-2"><i class="ti ti-shield-check text-(--color-success) mt-0.5" /> Files stay within tenant <span class="font-mono">@{{ tenantHandle }}</span>.</li>
              <li class="flex gap-2"><i class="ti ti-eye text-(--color-warning) mt-0.5" /> Review parsed fields before saving.</li>
            </ul>
          </div>
        </aside>
      </section>
        ...
        <div class="xl:col-span-2 space-y-8">
          ...
          <fieldset class="glass-card rounded-2xl p-6">
            <legend class="flex items-center justify-between w-full mb-4">
              ...
              <span class="text-xxs font-bold uppercase tracking-wider text-(--text-muted)">Step 1 of 4</span>
            </legend>
            ...
            <div class="form-grid">
              ...
              <div class="form-grid-full">
                ...
                <div class="input-with-icon">
                  ...
                  <input v-model="form.linkedin_url" type="url" class="form-control" placeholder="https://linkedin.com/in/...">
                </div>
              </div>
            </div>
          </fieldset>
          ...
          <fieldset class="glass-card rounded-2xl p-6">
            ...
            <div v-if="!form.work_experience.length" class="rounded-xl border border-dashed border-(--border-color) text-xxs text-(--text-muted) py-8 text-center">
              No experience entries yet. Add one with the button above.
            </div>

            <div v-else class="space-y-3">
              <div
                v-for="(exp, idx) in form.work_experience"
                :key="idx"
              ...
              >
                ...
                <div class="flex items-start gap-3 mb-3">
                  <div class="w-10 h-10 rounded-lg bg-(--color-primary-subtle) text-(--color-primary) inline-flex items-center justify-center shrink-0">
                    <i class="ti ti-building text-lg" />
                  </div>
                  <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input v-model="exp.title" type="text" class="form-control" placeholder="Job title">
                    <input v-model="exp.company" type="text" class="form-control" placeholder="Company">
                  </div>
                </div>
                ...
                <div class="form-grid">
                  <div>
                    ...
                    <input v-model="exp.period" type="text" class="form-control" placeholder="Jan 2021 – Present">
                  </div>
                  <div>
                    <label class="form-label">Location</label>
                    <input v-model="exp.location" type="text" class="form-control" placeholder="Remote">
                  </div>
                  <div class="form-grid-full">
                    <label class="form-label">Responsibilities</label>
                    <textarea v-model="exp.description" rows="3" class="form-control" placeholder="Key responsibilities and impact..." />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <fieldset class="glass-card rounded-2xl p-6">
              ...
              <div v-if="!form.education.length" class="rounded-lg border border-dashed border-(--border-color) text-xxs text-(--text-muted) py-6 text-center">
                ...
                >
                  ...
                  <div class="space-y-2">
                    <div>
                      ...
                      <input v-model="edu.degree" type="text" class="form-control" placeholder="B.S. Computer Science">
                    </div>
                    <div>
                      <label class="form-label">School</label>
                      <input v-model="edu.school" type="text" class="form-control" placeholder="University name">
                    </div>
                  </div>
                </div>
              </div>
            ...
            <fieldset class="glass-card rounded-2xl p-6">
              ...
              <div class="flex flex-wrap gap-2 mb-3">
                ...
                <span v-if="!form.skills.length" class="text-xxs text-(--text-muted) italic self-center">
                  No skills yet.
                </span>
              </div>
        ...
        <aside class="space-y-4 xl:sticky xl:top-20 self-start">
          <div class="glass-card rounded-2xl overflow-hidden">
            ...
            <div class="relative bg-(--bg-muted) h-[520px] overflow-auto custom-scrollbar">
              <div
                class="origin-top-left transition-transform mx-auto my-4"
                :style="{ transform: `scale(${zoom})`, width: 'fit-content' }"
              >
                <div v-if="!resumeFile" class="w-[360px] h-[480px] rounded-lg border border-dashed border-(--border-color) bg-(--bg-card) flex flex-col items-center justify-center gap-2 text-(--text-muted)">
                ...
                <div v-else class="w-[360px] rounded-lg border border-(--border-color) bg-white shadow-(--shadow-sm) overflow-hidden">
                  <div class="p-6 text-slate-700 text-xs space-y-4">
                    <div>
                      <p class="text-base font-bold text-slate-900">{{ form.applicant_name || 'Candidate name' }}</p>
                      <p class="text-slate-500">{{ form.applicant_email || '—' }} · {{ form.applicant_phone || '—' }}</p>
                      <p class="text-slate-500">{{ form.location || '—' }}</p>
                    </div>

                    <div v-if="form.work_experience.length">
                      <p class="uppercase tracking-widest text-[10px] font-bold text-slate-400 mb-1">Experience</p>
                      <div v-for="(exp, idx) in form.work_experience" :key="`pv-${idx}`" class="mb-2">
                        <p class="font-semibold text-slate-900">{{ exp.title || 'Role' }}</p>
                        <p class="text-slate-500">{{ exp.company || '—' }} · {{ exp.period || '—' }}</p>
                      </div>
                    </div>

                    <div v-if="form.education.length">
                      <p class="uppercase tracking-widest text-[10px] font-bold text-slate-400 mb-1">Education</p>
                    ...
                    <div v-if="form.skills.length">
                      <p class="uppercase tracking-widest text-[10px] font-bold text-slate-400 mb-1">Skills</p>
              ...
              <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-(--bg-card) rounded-full px-3 py-1.5 shadow-(--shadow-md) border border-(--border-color)">
                <button type="button" class="w-6 h-6 rounded-full hover:text-(--color-primary)"><i class="ti ti-chevron-left text-sm" /></button>
                <span class="text-xxs font-mono text-(--text-muted) px-2 border-x border-(--border-color)">Page 1 of 1</span>
                <button type="button" class="w-6 h-6 rounded-full hover:text-(--color-primary)"><i class="ti ti-chevron-right text-sm" /></button>
              </div>
            </div>
          </div>
        </aside>
      ...
      <section v-else-if="activeStep === 2" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="glass-card rounded-2xl p-6">
            <div class="flex items-center justify-between mb-5 pb-3 border-b border-(--border-color)">
              <div>
                ...
                <p class="text-xxs text-(--text-muted) mt-0.5">Verify everything below — then submit to enter the pipeline.</p>
              </div>
              <button type="button" class="btn btn-ghost text-xs" @click="goToStep(1)">
            ...
            <dl class="text-xs grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
              ...
              <div v-if="form.linkedin_url" class="sm:col-span-2">
                ...
                <dd>
                  <a :href="form.linkedin_url" target="_blank" class="text-(--color-primary) hover:underline font-mono break-all">{{ form.linkedin_url }}</a>
                </dd>
              </div>
              ...
              <div v-if="form.skills.length" class="sm:col-span-2">
                ...
                <dd class="flex flex-wrap gap-1.5">
                  <span v-for="skill in form.skills" :key="skill" class="px-2 py-0.5 rounded bg-(--color-primary-subtle) text-(--color-primary) font-semibold text-[10px]">
                    {{ skill }}
                  </span>
                </dd>
              </div>
              <div v-if="form.work_experience.length" class="sm:col-span-2">
                ...
                <dd class="space-y-1">
                  <div v-for="(exp, idx) in form.work_experience" :key="`fr-${idx}`" class="text-xxs text-(--text-body)">
                    <span class="font-semibold text-(--text-heading)">{{ exp.title || '—' }}</span> · {{ exp.company || '—' }} · {{ exp.period || '—' }}
                  </div>
                </dd>
              </div>
              <div v-if="form.education.length" class="sm:col-span-2">
                ...
                <dd class="space-y-1">
                  <div v-for="(edu, idx) in form.education" :key="`fe-${idx}`" class="text-xxs text-(--text-body)">
                    <span class="font-semibold text-(--text-heading)">{{ edu.degree || '—' }}</span> — {{ edu.school || '—' }}
                  </div>
                </dd>
              </div>
              <div v-if="form.cover_letter" class="sm:col-span-2">
                <dt class="form-label !mb-1">Cover letter</dt>
                <dd class="rounded-lg bg-(--bg-muted) p-3 text-(--text-body) whitespace-pre-wrap leading-relaxed">{{ form.cover_letter }}</dd>
              </div>
            </dl>
          </div>
        </div>
        ...
        <aside class="space-y-4">
          <div class="glass-card rounded-2xl p-5">
            <h4 class="text-xs font-bold uppercase tracking-widest text-(--text-muted) mb-3">Pipeline entry</h4>
            <p class="text-xs text-(--text-body) mb-3">
              On submit, the candidate is placed in the <span class="badge-soft-secondary text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full">Applied</span> column for
            ...
            <ul class="text-xxs text-(--text-muted) space-y-1.5">
              <li class="flex gap-2"><i class="ti ti-check text-(--color-success) mt-0.5" /> Audit entry recorded</li>
              <li class="flex gap-2"><i class="ti ti-check text-(--color-success) mt-0.5" /> Recruiter notified</li>
              <li class="flex gap-2"><i class="ti ti-check text-(--color-success) mt-0.5" /> Tenant scope: <span class="font-mono">@{{ tenantHandle }}</span></li>
            </ul>
          </div>

          <div v-if="resumeFile" class="glass-card rounded-2xl p-4 flex items-center gap-3">
            <span class="w-10 h-10 rounded-lg bg-(--color-danger-subtle) text-(--color-danger) inline-flex items-center justify-center shrink-0">
      ...
      <!-- Footer nav -->
      <footer class="sticky bottom-0 -mx-4 md:-mx-8 px-4 md:px-8 py-4 bg-(--header-bg) backdrop-blur-xl border-t border-(--border-color) flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 z-10">
        <button
          v-if="activeStep > 0"
          type="button"
          class="btn btn-ghost text-xs"
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          <button type="button" class="text-xs text-(--text-muted) hover:text-(--text-heading) px-3 py-2" :disabled="saving" @click="discard">
            Discard
          </button>
          <button
...
.step-pill:disabled { cursor: not-allowed; opacity: 0.6; }
.step-pill:hover:not(:disabled) { color: var(--text-heading); }
...
.step-pill-active { color: var(--color-primary); }
...
.step-pill-done { color: var(--color-primary); }
...
@media (min-width: 640px) {
  .step-bar { width: 2.5rem; }
}
.step-bar-done { background: rgb(var(--color-primary-rgb) / 0.4); }
