            ...
            <div class="flex flex-col md:flex-row gap-6">
                ...
                <aside class="w-full md:w-56 shrink-0">
                    <nav class="flex flex-col gap-1">
                        <button v-for="tab in tabs" :key="tab.key" type="button"
                            class="px-4 py-2.5 text-sm font-medium rounded-lg transition-colors flex items-center gap-3 text-left w-full"
                            :class="activeTab === tab.key
                                ? 'bg-(--color-primary-subtle) text-(--color-primary)'
                                : 'text-(--text-muted) hover:bg-(--bg-muted) hover:text-(--text-heading)'"
                            @click="activeTab = tab.key">
                            <i :class="['ti', tab.icon, 'text-[1.15rem]']" />
                            {{ tab.label }}
                        </button>
                    </nav>
                </aside>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <section v-else-if="activeTab === 'locale'" class="glass-card rounded-2xl p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input v-model="draft['locale.timezone']" type="text" placeholder="UTC"
                                    class="form-control" />
                            </div>
                            <div>
                                <label
                    ...
                    <section v-else-if="activeTab === 'notifications'" class="glass-card rounded-2xl p-6 space-y-6">
                        ...
                        <label
                            class="flex items-center justify-between p-4 rounded-xl border border-(--border-color) cursor-pointer">
                            <div>
                                <p class="text-sm font-semibold text-(--text-heading)">Send transactional emails</p>
                                <p class="text-xxs text-(--text-muted) mt-0.5">Approvals, payslip releases, password
                                    resets.</p>
                            </div>
                    ...
                    <!-- Numbering -->
                    <section v-else-if="activeTab === 'numbering'" class="glass-card rounded-2xl p-6 space-y-6">
                        <header class="flex items-center gap-2 pb-3 border-b border-(--border-color)">
                            <i class="ti ti-hash text-(--color-primary)" />
                            <div>
                                <h3 class="text-sm font-semibold">Document Numbering Prefixes</h3>
                                    ...
                                    <div v-for="(mod, index) in group.items" :key="mod.id"
                                        ...
                                        <div v-if="mod.children?.length"
                                            ...
                                            <div v-for="(child, childIdx) in mod.children" :key="child.id"
                                                ...
                                                class="flex items-center gap-3 px-6 py-2.5 pl-16 border-b border-(--border-color)/40 last:border-b-0 transition-all duration-200 cursor-grab active:cursor-grabbing"
                                                :class="[
...
const ALL_TABS = [
    ...
    { key: 'numbering', label: 'Numbering', icon: 'ti-hash', adminOnly: false },
...
const dirty = computed(() =>
    Object.keys(draft).some(k => !valuesEqual(draft[k], pristine.value[k])) || modulesDirty.value
)
        ...
        if (modulesDirty.value) {
            ...
            for (const group of moduleGroups.value) {
                for (const mod of group.items) {
                    payload.push({ id: mod.id, is_active: mod.isActive, sort_order: ++sortOrder })
                    if (mod.children) {
                        for (const child of mod.children) {
                            payload.push({ id: child.id, is_active: child.isActive, sort_order: ++sortOrder })
                        }
                    }
                }
            }
...
const onDragEnter = (e: DragEvent, overId: string, groupKey: string) => {
    ...
    if (fromIdx !== -1 && toIdx !== -1) {
        // Splice the array during hover so the DOM reflows instantly,
        // and the 'dragged' item itself acts as the skeleton placeholder.
...
const onChildDragEnter = (e: DragEvent, overChildId: string, parentId: string) => {
    ...
    const parentModule = allModules.value.find(m => m.id === parentId)
