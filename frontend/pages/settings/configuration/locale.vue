    ...
    <NuxtLayout name="default">
        <div class="space-y-6">
            ...
            <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    <h1 class="text-xl font-semibold">Locale Configuration</h1>
                    <p class="text-xs text-(--text-muted) mt-1">
                        Manage timezone, language, and formatting defaults for
...
definePageMeta({
    breadcrumb: 'Locale Configuration'
})
...
const save = async () => {
    ...
    try {
        ...
        if (changed.length) {
            ...
            hydrate(data)
            alert.msg = 'Locale configuration saved successfully'
            alert.type = 'success'
        }
    } catch (err: any) {
