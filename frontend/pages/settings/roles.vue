        ...
        <div class="space-y-6">
            ...
            <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    ...
                    <p class="text-xs text-(--text-muted) mt-1">Configure access groups and assign granular API scopes.
                    </p>
                </div>
            ...
            <!-- Alert -->
            <div v-if="alertMsg" class="px-4 py-3 rounded-lg flex items-center justify-between text-xs font-semibold"
            ...
            <section class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <aside class="glass-card rounded-2xl p-5">
                    ...
                    <div v-if="loading" class="py-12 flex justify-center">
                        <span
                            class="w-6 h-6 rounded-full border-2 border-(--color-primary)/20 border-t-(--color-primary) animate-spin" />
                    </div>
                            ...
                            <button type="button"
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    <span class="text-xs font-semibold"
                                        ...
                                        role.name }}</span>
                                    <span
                                        class="text-xxs font-mono px-1.5 py-0.5 rounded border border-(--border-color) text-(--text-muted) uppercase">{{
                                        role.slug }}</span>
                                </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div v-if="!selectedRole" class="glass-card rounded-2xl py-16 text-center">
                        ...
                        <h4 class="text-sm font-semibold text-(--text-heading) mt-3">Select a security group to inspect
                        </h4>
                        <p class="text-xs text-(--text-muted) mt-1">Toggle granular permissions to commit policy
                            updates.</p>
                    </div>

                    <div v-else class="glass-card rounded-2xl p-6 space-y-6">
                        <header
                            class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-5 border-b border-(--border-color)">
                            <div>
                                <h3>Role policy: <span class="text-(--color-primary)">{{ selectedRole.name }}</span>
                                </h3>
                                <p class="text-xs text-(--text-muted) mt-1">Toggle permissions and commit to sync with
                                    the database.</p>
                            </div>
                            <div class="flex gap-2">
                                <button v-if="selectedRole.slug !== 'admin'"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label
                                    class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Role
                                    name</label>
                                <input v-model="selectedRole.name" type="text" class="form-control" />
                            </div>
                            <div>
                                <label
                                    class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Description</label>
                                <input v-model="selectedRole.description" type="text" class="form-control" />
                            </div>
                        </div>

                        <div class="space-y-5">
                            <div v-for="(features, moduleName) in groupedPermissions" :key="moduleName"
                                class="space-y-3">
                                <h5
                                    class="flex items-center gap-2 text-xxs font-bold uppercase tracking-widest text-(--text-muted)">
                                    <span class="w-1.5 h-3.5 rounded-sm bg-(--color-primary)" />
                                    {{ moduleName }} module
                                </h5>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div v-for="(actions, featureName) in features" :key="featureName"
                                        class="p-4 rounded-xl bg-(--bg-muted)/60 border border-(--border-color)">
                                        <h6 class="text-xs font-semibold text-(--text-heading) capitalize mb-3">{{
                                            featureName }} management</h6>
                                        <ul class="space-y-1.5">
                                            <li v-for="perm in actions" :key="perm.id">
                                                <label
                                                    class="flex items-center justify-between cursor-pointer py-1 px-1.5 rounded hover:bg-(--bg-card)">
                                                    <span class="min-w-0 pr-3">
                                                        <span class="block text-xs text-(--text-heading)">{{ perm.name
                                                            }}</span>
                                                        <span class="block text-xxs font-mono text-(--text-muted)">{{
                                                            perm.slug }}</span>
                                                    </span>
                    ...
                    <form @submit.prevent="createRole" class="space-y-4">
                        <div>
                            <label
                                class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Role
                                name</label>
                            <input v-model="newRoleForm.name" type="text" required class="form-control"
                                placeholder="Sales Representative" />
                        </div>
                        <div>
                            <label
                                class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">System
                                slug</label>
                            <input v-model="newRoleForm.slug" type="text" required class="form-control"
                                placeholder="sales-rep" />
                        </div>
                        <div>
                            <label
                                class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Description</label>
                            <textarea v-model="newRoleForm.description" rows="3" class="form-control resize-none"
                        ...
                        <footer class="pt-4 border-t border-(--border-color) flex justify-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary text-xs"><i class="ti ti-check" />Establish
                                role</button>
                        </footer>
                    </form>
                </div>
