            ...
            <nav class="flex flex-wrap gap-2 border-b border-(--border-color)">
                <button v-for="tab in tabs" :key="tab.key" type="button"
                    ...
                    :class="activeTab === tab.key
                        ...
                        : 'text-(--text-muted) border-transparent hover:text-(--text-heading)'" @click="activeTab = tab.key">
                    <i :class="['ti', tab.icon]" />
                    {{ tab.label }}
                </button>
            </nav>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button v-for="swatch in accentSwatches" :key="swatch.rgb" type="button"
                                class="w-9 h-9 rounded-full border-2 transition-transform hover:scale-110" :class="draft['branding.primary_color'] === swatch.rgb
                                    ? 'border-(--text-heading)' : 'border-transparent'" :style="{ background: `rgb(${swatch.rgb})` }"
                                :title="swatch.label" @click="setBranding('primary_color', swatch.rgb)" />
                        </div>
            ...
            <section v-else-if="activeTab === 'modules'" class="space-y-4">
                <div class="glass-card rounded-2xl p-5 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-(--text-muted) mt-1">Enable or disable modules for this tenant. Core
                            ...
                            <div v-for="mod in group.items" :key="mod.id">
                                ...
                                <div class="flex items-center gap-4 px-6 py-3.5 transition-colors"
                                    ...
                                    <button type="button"
                                        ...
                                        ]" :disabled="mod.isCore || togglingId === mod.id" @click="toggleModule(mod)">
                                        <span
                                ...
                                <div v-if="mod.children?.length"
                                    ...
                                    <div v-for="child in mod.children" :key="child.id"
