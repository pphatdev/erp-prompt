        ...
        <div class="flex gap-4 min-w-max">
          ...
          >
            <header class="flex items-center justify-between px-1">
              ...
              <span
                v-if="movingId && col.status === pendingDropStatus"
            ...
            <div class="kanban-list flex flex-col gap-3 pr-1">
              ...
              >
                <header class="flex items-start justify-between gap-2 mb-3">
                  <div class="flex items-center gap-2 min-w-0">
                    <div
                      class="w-8 h-8 rounded-full flex items-center justify-center text-xxs font-bold text-(--color-primary) bg-(--color-primary-subtle) border border-(--color-primary)/20 shrink-0"
                  ...
                  <Badge
                    v-if="cardBadge(a, col.status).show"
                ...
                <div
                  v-if="col.status === 'offer' && canSeeSalary && a.expectedSalary != null"
                ...
                >
                  <NuxtLink
                    v-if="a.employeeId"
                    :to="`/employees?id=${a.employeeId}`"
                    class="hired-chip hired-chip--linked"
                    draggable="false"
                  >
                  ...
                  <button
                    ...
                    type="button"
                    class="hired-chip hired-chip--cta"
                    :disabled="converting === a.id"
                    @click="convertApplicationToEmployee(a)"
                  >
                    <i :class="['ti text-[12px]', converting === a.id ? 'ti-loader animate-spin' : 'ti-user-plus']" />
                  ...
                  >
                    <i :class="['ti text-[11px]', reverting === a.id ? 'ti-loader animate-spin' : 'ti-arrow-back-up']" />
                ...
                <footer class="flex items-center justify-between text-xxs text-(--text-muted)">
                  ...
                  >
                    <i :class="['ti text-[11px]', isOverdue(a) ? 'ti-alert-triangle' : 'ti-clock']" />
              ...
              <template v-if="!grouped[col.status]?.length">
                <NuxtLink
                  v-if="col.status === 'applied' && canWrite"
        ...
        <div class="glass-card rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 shadow-(--shadow-lg) bg-(--bg-card)">
          ...
          <form class="form-grid" @submit.prevent="submitApplication">
            ...
            <footer class="form-grid-full pt-4 border-t border-(--border-color) flex justify-end gap-2">
              <button type="button" class="btn btn-ghost text-xs" @click="closeSubmitModal">Cancel</button>
...
.hired-chip:active { transform: scale(0.98); }
.hired-chip:disabled { opacity: 0.6; cursor: not-allowed; }
...
.hired-chip--cta:hover:not(:disabled) { background: rgb(var(--color-primary-rgb) / 0.9); }
...
.hired-revert:disabled { opacity: 0.6; cursor: not-allowed; }
