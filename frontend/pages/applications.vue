    ...
    <NuxtLayout name="default">
        <div class="space-y-6">
            ...
            <section v-else class="glass-card rounded-2xl overflow-hidden">
                ...
                <transition name="bulkbar">
                    <div v-if="selectedCount > 0" class="bulk-toolbar">
                        ...
                        <div class="flex items-center gap-2">
                            <button v-if="canConvert" type="button" class="btn btn-soft-primary text-xs px-3 py-1.5"
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr
                                ...
                                <th v-if="canSeeSalary" class="px-4 py-3 font-semibold font-mono text-right">Expected
                                </th>
                        ...
                        <tbody class="divide-y divide-(--border-color)">
                            <tr v-for="a in applications" :key="a.id" class="transition-colors"
                                :class="selectedIds.has(a.id) ? 'bg-(--color-primary-subtle)/30' : 'hover:bg-(--bg-muted)'">
                                <td class="pl-4 pr-1 py-3 w-8">
                                    <input type="checkbox" class="row-checkbox" :checked="selectedIds.has(a.id)"
                ...
                <div
                    ...
                    <form class="form-grid" @submit.prevent="submitEdit">
                        ...
                        <div class="form-grid-full">
                            ...
                            <input v-model="editForm.linkedin_url" type="url" class="form-control"
                        ...
                        <div>
                            ...
                            <input v-model.number="editForm.expected_salary" type="number" min="0" step="0.01"
                        ...
                        <div class="form-grid-full">
                            ...
                            <input v-model="editForm._skillsText" type="text" class="form-control"
                        ...
                        <div class="form-grid-full">
                            ...
                            <textarea v-model="editForm.notes" rows="3" class="form-control"
                                placeholder="Internal recruiter notes (visible to staff only)." />
                        </div>
            ...
            <div v-if="detailsOpen"
                class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div
                    class="glass-card rounded-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto p-6 shadow-(--shadow-lg) bg-(--bg-card)">
                    <header class="flex items-center justify-between mb-5">
                        <div>
                            <h3 class="text-base font-semibold text-(--text-heading)">{{ detailsApp?.applicantName }}
                            </h3>
                            <p class="text-xxs text-(--text-muted) mt-1">
                                <span v-if="detailsApp?.candidateCode" class="font-mono text-(--color-primary)">{{
                                    detailsApp.candidateCode }}</span>
                    ...
                    <dl class="text-xs space-y-3">
                        ...
                        <div class="flex justify-between gap-3">
                            ...
                            <dd class="text-(--text-body)">
                                <a v-if="detailsApp?.linkedinUrl" :href="detailsApp.linkedinUrl" target="_blank"
                                    class="text-(--color-primary) hover:underline font-mono">
                                    View Profile
                                </a>
                                <span v-else>—</span>
                            </dd>
                        ...
                        <div v-if="canSeeSalary" class="flex justify-between gap-3">
                            ...
                            <dd class="text-(--text-body) font-mono">{{ detailsApp?.expectedSalary != null ?
                                formatMoney(detailsApp.expectedSalary) : '—' }}</dd>
                        </div>
                        <div class="flex justify-between gap-3">
                            <dt class="text-(--text-muted) uppercase tracking-widest text-xxs font-bold">Applied</dt>
                            <dd class="text-(--text-body) font-mono">{{ formatDateTime(detailsApp?.appliedAt || null) }}
                            </dd>
                        </div>
                        <div v-if="detailsApp?.resumePath" class="flex justify-between gap-3">
                            ...
                            <dd class="text-(--color-primary) font-mono truncate max-w-[60%]"
                                :title="detailsApp.resumePath">{{ detailsApp.resumePath }}</dd>
                        </div>
                        <div v-if="detailsApp?.skills?.length" class="flex flex-col gap-1.5 pt-2">
                            ...
                            <dd class="flex flex-wrap gap-1">
                                <span v-for="skill in detailsApp.skills" :key="skill"
                                    class="px-2 py-0.5 rounded bg-(--color-primary-subtle) text-(--color-primary) font-semibold text-[10px]">
                                    {{ skill }}
                                </span>
                            </dd>
                        </div>
                        <div v-if="detailsApp?.education?.length"
                            class="flex flex-col gap-1.5 pt-2 border-t border-(--border-color)">
                        ...
                        <div v-if="detailsApp?.workExperience?.length"
                            class="flex flex-col gap-1.5 pt-2 border-t border-(--border-color)">
                            <dt class="text-(--text-muted) uppercase tracking-widest text-xxs font-bold">Work Experience
                            </dt>
                            <dd class="space-y-3">
                                <div v-for="(exp, index) in detailsApp.workExperience" :key="index"
                                    class="text-xxs space-y-0.5">
                                    ...
                                    <p class="text-(--text-body) mt-1 leading-normal whitespace-pre-line">{{
                                        exp.description }}</p>
                                </div>
                            </dd>
                        </div>
                        <div v-if="detailsApp?.coverLetter" class="pt-2 border-t border-(--border-color)">
                            <dt class="text-(--text-muted) uppercase tracking-widest text-xxs font-bold mb-1">Cover
                                letter</dt>
                            <dd class="rounded-lg bg-(--bg-muted) p-3 text-(--text-body) whitespace-pre-wrap">{{
                                detailsApp.coverLetter }}</dd>
                        </div>
                    </dl>
            ...
            <!-- Action dropdown -->
            <div v-if="actionMenu.open && actionMenu.app"
                ...
                <button class="action-item" @click="actionOpenProfile">
