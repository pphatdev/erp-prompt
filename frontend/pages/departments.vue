    ...
    <div class="space-y-6">
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p class="text-xs text-(--text-muted) mt-1">Org units used to group employees and route approvals.</p>
        </div>
      ...
      <section class="glass-card rounded-xl p-4">
        <div class="relative w-full md:w-96">
          ...
          <input v-model="search" type="search" placeholder="Search by name or code..." class="form-control pl-9" />
        </div>
      </section>
        ...
        <div class="glass-card rounded-2xl w-full max-w-md p-6 shadow-(--shadow-lg) bg-(--bg-card)">
          ...
          <form class="space-y-4" @submit.prevent="saveDepartment">
            <div>
              ...
              <input v-model="form.name" type="text" required class="form-control" placeholder="Engineering" />
            </div>
            <div>
              <label class="form-label">Code</label>
              <input v-model="form.code" type="text" required class="form-control font-mono uppercase" placeholder="ENG" />
            </div>
