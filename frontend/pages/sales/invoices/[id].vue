<template>
  <NuxtLayout name="default">
    ...
    <div v-else-if="invoice" class="space-y-6">
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p class="text-xs text-(--text-muted) mt-1">
            ...
            ·
            <NuxtLink :to="`/sales/orders/${invoice.orderId}`" class="text-(--color-primary) hover:underline">From order</NuxtLink>
          </p>
        </div>

        <div class="flex flex-wrap gap-2">
          <button v-if="invoice.status === 'new'" class="btn btn-primary text-xs" :disabled="acting" @click="confirm">
            <i class="ti ti-check" />Confirm &amp; post to GL
          </button>
          <button v-if="invoice.status === 'new'" class="btn text-xs text-(--color-danger) border border-(--color-danger)/20 hover:bg-(--color-danger-subtle)"
      ...
      <div v-if="invoice.status === 'new'" class="px-4 py-3 rounded-lg bg-(--color-warning-subtle) text-(--color-warning) text-xs">
      ...
      <section v-if="invoice.journalEntryId"
        ...
        <div>
          ...
          <p class="text-sm font-mono font-semibold text-(--text-heading) mt-1">{{ invoice.journalEntryId }}</p>
      ...
      <section class="glass-card rounded-2xl p-5">
        ...
        <div class="overflow-x-auto -mx-5">
          <table class="w-full text-xs">
            ...
            <tbody>
              <tr v-for="line in invoice.items" :key="line.id" class="border-b border-(--border-color)/40">
    ...
    <div v-if="showCancel" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="glass-card rounded-2xl w-full max-w-md bg-(--bg-card) shadow-(--shadow-lg)">
        ...
        <div class="p-5 space-y-3">
          <p class="text-xs text-(--text-muted)">Only invoices in <code class="font-mono">new</code> status can be cancelled here. Confirmed invoices need a credit-note reversal via FMS.</p>
