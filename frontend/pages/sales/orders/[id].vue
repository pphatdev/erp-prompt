<template>
  <NuxtLayout name="default">
    ...
    <div v-else-if="order" class="space-y-6">
        ...
        <div>
          ...
          <p class="text-xs text-(--text-muted) mt-1">
            ...
            <span v-if="order.quotationId" class="ml-2">·
              <NuxtLink :to="`/sales/quotations/${order.quotationId}`" class="text-(--color-primary) hover:underline">From quote</NuxtLink>
            </span>
          </p>
        </div>

        <div class="flex flex-wrap gap-2">
          <button v-if="order.status === 'new'" class="btn btn-primary text-xs" :disabled="acting" @click="confirm">
            <i class="ti ti-check" />Confirm &amp; fulfill
          </button>
          <button v-if="order.status === 'new'" class="btn text-xs text-(--color-danger) border border-(--color-danger)/20 hover:bg-(--color-danger-subtle)"
      ...
      <!-- Confirm warning -->
      <div v-if="order.status === 'new'" class="px-4 py-3 rounded-lg bg-(--color-warning-subtle) text-(--color-warning) text-xs">
        <i class="ti ti-alert-triangle" />
        Confirming this order will <b>atomically</b> create an Invoice, set up a Subscription for software lines, and deduct hardware lines from stock. Anything that fails rolls the entire order back to <code class="font-mono">new</code>.
      </div>
    ...
    <div v-if="showCancel" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="glass-card rounded-2xl w-full max-w-md bg-(--bg-card) shadow-(--shadow-lg)">
        ...
        <div class="p-5 space-y-3">
          <p class="text-xs text-(--text-muted)">Only orders in <code class="font-mono">new</code> status can be cancelled. Confirmed orders require reversing their downstream artifacts first.</p>
