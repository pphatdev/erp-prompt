        ...
        <div v-else-if="customer" class="space-y-6">
                ...
                <div class="flex items-center gap-4 min-w-0">
                    ...
                    <div class="min-w-0">
                        <div class="flex items-center flex-wrap gap-2">
                            ...
                            <Badge :variant="customer.status === 'active' ? 'success' : 'secondary'">{{ customer.status
                                }}</Badge>
                        ...
                        <p class="text-xs text-(--text-muted) truncate mt-0.5">{{ customer.companyName || customer.email
                            }}</p>
                    </div>
                </div>
            ...
            <section class="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div class="glass-card rounded-xl p-4">
                    ...
                    <p class="text-base font-semibold text-(--text-heading) mt-1">{{ formatDate(customer.createdAt) }}
                    </p>
                </div>
            </section>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-4">
                            ...
                            <div>
                                ...
                                <dd class="detail-value">
                                    <a v-if="customer.website" :href="customer.website" target="_blank" rel="noopener"
                                        class="text-(--color-primary) hover:underline truncate block">{{
                                        customer.website }}</a>
                                    <span v-else>—</span>
                                </dd>
                            </div>
                    ...
                    <section class="glass-card rounded-2xl p-5">
                        ...
                        <div v-if="!customer.orders?.length" class="text-xs text-(--text-muted) italic mt-3">No orders
                            yet.</div>
                            ...
                            <table class="w-full text-xs">
                                ...
                                <tbody>
                                    <tr v-for="o in customer.orders" :key="o.id"
                                        ...
                                        <td class="px-5 py-3">
                                            <NuxtLink :to="`/sales/orders/${o.id}`"
                                                class="text-(--color-primary) hover:underline font-mono font-semibold">
                                                {{ o.orderNumber }}
                                            </NuxtLink>
                                        </td>
                                        <td class="px-2 py-3">
                                        ...
                                        <td class="px-2 py-3 text-right font-mono font-semibold">{{
                                            fmtMoney(o.totalAmount) }}</td>
                                        <td class="px-5 py-3 text-right text-(--text-muted)">{{ formatDate(o.createdAt)
                                            }}</td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <section v-if="customer.leads?.length" class="glass-card rounded-2xl p-5">
                        ...
                        <div class="overflow-x-auto -mx-5 mt-3">
                            <table class="w-full text-xs">
                                ...
                                <tbody>
                                    <tr v-for="l in customer.leads" :key="l.id"
                                        class="border-b border-(--border-color)/40">
                ...
                <div class="space-y-4">
                    ...
                    <section v-if="customer.notes" class="glass-card rounded-2xl p-5">
                        ...
                        <p class="text-xs text-(--text-body) whitespace-pre-line mt-3 leading-relaxed">{{ customer.notes
                            }}</p>
                    </section>
                    ...
                    <section class="glass-card rounded-2xl p-5">
                        ...
                        <div v-if="customer.accountManager" class="flex items-center gap-3 mt-4">
                            <div
                                class="w-9 h-9 rounded-full bg-(--color-primary-subtle) text-(--color-primary) flex items-center justify-center font-bold text-sm">
                                {{ customer.accountManager.name.charAt(0).toUpperCase() }}
                            </div>
                            <div>
                                <p class="text-xs font-semibold text-(--text-heading)">{{ customer.accountManager.name
                                    }}</p>
                    ...
                    <!-- Branding -->
                    <section v-if="customer.brandPrimaryColor || customer.brandLogoUrl"
                        ...
                        <div class="space-y-3 mt-4">
                            <div v-if="customer.brandPrimaryColor">
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    ...
                                    <code
                                        class="text-xxs text-(--text-muted)">rgb({{ customer.brandPrimaryColor }})</code>
                                </div>
                            </div>
                            <div v-if="customer.brandLogoUrl">
                                <p class="detail-label">Logo</p>
                                <img :src="customer.brandLogoUrl"
                                    class="mt-1 h-10 object-contain rounded border border-(--border-color) bg-(--bg-muted) p-1"
                                    alt="Brand logo" />
                            </div>
                        </div>
                    </section>
                    ...
                    <section v-if="customer.customerType === 'tenant'" class="glass-card rounded-2xl p-5">
                        ...
                        <div class="space-y-4 mt-4">
                            ...
                            <!-- Pending state -->
                            <div v-else
                                class="rounded-xl border border-(--border-color) bg-(--bg-muted) p-4 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xxs text-(--text-muted) mt-0.5">
                                        A tenant database and subdomain will be created automatically when a
                                        subscription
                                        for this customer is confirmed.
                                    </p>
                                    <p v-if="customer.tenantHandle"
                                        class="text-xxs font-mono text-(--color-primary) mt-1.5">
                            ...
                            <!-- Metadata row -->
                            <dl v-if="customer.tenantHandle || customer.provisionedAt"
                                class="grid grid-cols-2 gap-3 text-xxs">
