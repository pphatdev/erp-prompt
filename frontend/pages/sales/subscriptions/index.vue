    ...
    <div class="space-y-6">
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p class="text-xs text-(--text-muted) mt-1">Software-fulfillment side of a Sales Order. Confirming dispatches the tenant-provisioning event.</p>
        </div>
      </header>
      ...
      <section class="glass-card rounded-xl p-4 flex flex-col md:flex-row gap-3 items-center justify-between">
        <div class="relative w-full md:w-80">
          ...
          <input v-model="search" type="search" placeholder="Search subscription # or customer..." class="form-control pl-9" />
        </div>
      ...
      <div v-else-if="filtered.length === 0" class="glass-card rounded-2xl py-20 text-center">
        ...
        <p class="text-xs text-(--text-muted) mt-1">Subscriptions are created automatically when a confirmed order contains software lines.</p>
      </div>
      ...
      <section v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <NuxtLink v-for="s in filtered" :key="s.id" :to="`/sales/subscriptions/${s.id}`"
          ...
          <div class="flex items-center gap-2 text-xxs">
            <span class="px-2 py-0.5 rounded badge-soft-secondary font-mono uppercase tracking-widest">{{ s.billingCycle }}</span>
