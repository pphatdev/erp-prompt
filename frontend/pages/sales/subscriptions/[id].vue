<template>
  <NuxtLayout name="default">
    ...
    <div v-else-if="sub" class="space-y-6">
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p class="text-xs text-(--text-muted) mt-1">
            ...
            ·
            <NuxtLink :to="`/sales/orders/${sub.orderId}`" class="text-(--color-primary) hover:underline">From order</NuxtLink>
          </p>
        </div>

        <div class="flex flex-wrap gap-2">
          <button v-if="sub.status === 'new'" class="btn btn-primary text-xs" :disabled="acting" @click="confirm">
            <i class="ti ti-cloud-upload" />Confirm &amp; provision
          </button>
          <button v-if="sub.status !== 'cancelled'" class="btn text-xs text-(--color-danger) border border-(--color-danger)/20 hover:bg-(--color-danger-subtle)"
      ...
      <div v-if="sub.status === 'new'" class="px-4 py-3 rounded-lg bg-(--color-info-subtle) text-(--color-info) text-xs">
        <i class="ti ti-info-circle" />
        Confirming dispatches the <code class="font-mono">SubscriptionConfirmed</code> event. The provisioning listener will create the customer's tenant, seed default IAM, create their first admin user, and email an activation link.
      </div>

      <section class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="glass-card rounded-xl p-4">
          <p class="text-xxs text-(--text-muted) uppercase tracking-widest font-bold">Cycle</p>
          <p class="text-base font-semibold text-(--text-heading) mt-1 capitalize">{{ sub.billingCycle.replace('_', ' ') }}</p>
        </div>
        <div class="glass-card rounded-xl p-4">
      ...
      <section v-if="sub.provisionedTenantId || sub.provisionedAt"
        ...
        <div>
          ...
          <p class="text-sm font-mono font-semibold text-(--text-heading) mt-1">{{ sub.provisionedTenantId || 'Pending listener' }}</p>
          <p v-if="sub.provisionedAt" class="text-xxs text-(--text-muted) mt-0.5">at {{ formatDate(sub.provisionedAt) }}</p>
        </div>
    ...
    <!-- Cancel modal -->
    <div v-if="showCancel" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="glass-card rounded-2xl w-full max-w-md bg-(--bg-card) shadow-(--shadow-lg)">
        <header class="flex items-center justify-between p-5 border-b border-(--border-color)">
          ...
          <button class="w-8 h-8 rounded-full hover:bg-(--bg-muted)" @click="showCancel = false"><i class="ti ti-x" /></button>
        </header>
        <div class="p-5 space-y-3">
          <p class="text-xs text-(--text-muted)">Cancelling deactivates the subscription. The customer's tenant remains until deprovisioning lands (out of scope for now).</p>
          <input v-model="cancelReason" type="text" maxlength="500" placeholder="Reason (optional)" class="form-control text-xs" />
        </div>
