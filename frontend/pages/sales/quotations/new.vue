      ...
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-(--text-muted) mt-0.5">Build a quote, confirm it, then convert it to a sales order.</p>
          </div>
        </div>
        <div class="flex gap-2">
          ...
          <button type="button" class="btn btn-primary text-xs" :disabled="submitting || !canSubmit" @click="submit">
        ...
        <section class="glass-card rounded-2xl p-6 lg:col-span-2 space-y-6">
          ...
          <div>
              ...
              <div v-for="(line, idx) in form.items" :key="idx"
                ...
                <div class="grid grid-cols-3 md:grid-cols-4 gap-3 items-end">
                  ...
                  <div>
                    ...
                    <input v-model.number="line.discount_pct" type="number" min="0" max="100" step="0.1"
                      class="form-control text-xs" placeholder="0" />
                  </div>
            ...
            <div v-if="form.items.length === 0"
              ...
              <p class="text-xs text-(--text-muted) mt-2">No lines yet — click <strong>Add line</strong> to start.</p>
            </div>
          </div>
        </section>
          ...
          <div class="glass-card rounded-2xl p-5">
            ...
            <dl class="space-y-2 text-xs">
              ...
              <div class="flex justify-between border-t border-(--border-color) pt-2 mt-2">
                ...
                <dd class="text-base font-bold text-(--text-heading) font-mono">{{ fmt(grandTotal) }}</dd>
              </div>
            </dl>
          </div>
          ...
          <div class="glass-card rounded-2xl p-5">
            ...
            <ul class="space-y-2">
              ...
              <li class="flex items-center gap-2 text-xs"
                ...
                <i :class="['ti', form.items.length > 0 ? 'ti-circle-check-filled' : 'ti-circle-dashed']" />
                At least one line
              </li>
              <li class="flex items-center gap-2 text-xs"
