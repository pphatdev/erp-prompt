    ...
    <div v-else-if="quote" class="space-y-6">
      ...
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <button v-if="quote.status !== 'cancelled' && !quote.orderId" class="btn text-xs text-(--color-danger) border border-(--color-danger)/20 hover:bg-(--color-danger-subtle)"
            :disabled="acting" @click="cancelConfirm">
            <i class="ti ti-ban" />Cancel
          </button>
          <NuxtLink v-if="quote.orderId" class="btn btn-soft-primary text-xs" :to="`/sales/orders/${quote.orderId}`">
    ...
    <div v-if="showCancel" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="glass-card rounded-2xl w-full max-w-md bg-(--bg-card) shadow-(--shadow-lg)">
        ...
        <div class="p-5 space-y-3">
          <p class="text-xs text-(--text-muted)">This will close the lead. The quote stays in the audit trail but can't be confirmed afterwards.</p>
