        ...
        <div class="space-y-6">
            <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    ...
                    <p class="text-xs text-(--text-muted) mt-1">Catalogue of leave categories with annual allowance per
                        employee.</p>
                </div>
            ...
            <div v-else-if="types.length === 0" class="glass-card rounded-2xl py-20 text-center">
                ...
                <p class="text-xs text-(--text-muted) mt-1">Define categories such as Annual, Sick, Maternity before
                    employees can request leave.</p>
            </div>
            ...
            <section v-else class="glass-card rounded-2xl overflow-hidden">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-(--border-color)">
                            <tr v-for="t in types" :key="t.id" class="hover:bg-(--bg-muted) transition-colors">
                                ...
                                <td class="px-4 py-3 font-mono text-xxs text-(--text-muted)">{{ formatDate(t.createdAt)
                                    }}</td>
                ...
                <Pagination :page="pagination.page" :limit="pagination.limit" :total="pagination.total"
                    :total-pages="pagination.totalPages" @update:page="(p) => { pagination.page = p; loadTypes() }"
