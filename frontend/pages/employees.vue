    ...
    <div class="space-y-6">
      ...
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p class="text-xs text-(--text-muted) mt-1">
            <span v-if="pagination.total">
              Manage {{ pagination.total.toLocaleString() }} workforce records — hire, edit, and offboard personnel.
            </span>
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="inline-flex items-center bg-(--bg-card) border border-(--border-color) rounded-lg p-1">
            <button
              v-for="opt in (['table', 'grid'] as const)"
              :key="opt"
              type="button"
      ...
      <section class="glass-card rounded-xl p-4">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
          <div class="relative md:col-span-5">
            ...
            <input v-model="filters.search" type="search" placeholder="Search name, email, or ID..." class="form-control pl-9" />
          </div>

          <div class="relative md:col-span-3">
            <i class="ti ti-building absolute left-3 top-1/2 -translate-y-1/2 text-(--text-muted) text-sm pointer-events-none" />
          ...
          <div class="md:col-span-4 flex items-center border border-(--border-color) rounded-lg bg-(--bg-muted) p-1">
            <button
              v-for="s in (['', 'active', 'on_leave', 'terminated'] as const)"
              :key="s || 'all'"
      ...
      <section v-else-if="view === 'table'" class="glass-card rounded-2xl overflow-hidden">
        ...
        <Pagination
          ...
          @update:page="(p) => { pagination.page = p; loadEmployees() }"
          ...
          >
            ...
            <header class="flex items-start justify-between gap-3 mb-4">
              <div class="flex items-start gap-3 min-w-0">
                <div class="relative shrink-0">
                  <div
                    class="w-12 h-12 rounded-xl bg-(--color-primary-subtle) text-(--color-primary) flex items-center justify-center font-semibold text-sm"
                    :title="emp.fullName"
                  >
                    {{ initials(emp) }}
                  </div>
              ...
              <button
                v-if="canWrite || emp.status !== 'terminated'"
                type="button"
            ...
            <dl class="space-y-2 mb-5 text-xs">
              ...
              <div class="flex items-center gap-2 text-(--text-body)">
                ...
                <span class="font-mono text-xxs">{{ emp.hiredAt ? `Hired ${formatRelativeDate(emp.hiredAt)}` : 'Hire date —' }}</span>
              </div>
              <div class="flex items-center gap-2 text-(--text-body)">
      ...
      <div
        v-if="cardMenu.open && cardMenu.emp"
        ...
        <div class="glass-card rounded-2xl w-full max-w-lg p-6 shadow-(--shadow-lg) bg-(--bg-card)">
          ...
          <form class="grid grid-cols-1 sm:grid-cols-2 gap-4" @submit.prevent="saveEmployee">
            <div class="sm:col-span-2">
              ...
              <input v-model="form.employee_id" type="text" required class="form-control font-mono" placeholder="EMP-0001" />
            </div>
            ...
            <div v-if="canSeeSalary">
              ...
              <input v-model.number="form.base_salary" type="number" step="0.01" min="0" class="form-control font-mono" />
            </div>
            <div>
...
.emp-card-kebab:hover { background: var(--bg-muted); color: var(--text-heading); }
.emp-card-kebab--open { background: var(--bg-muted); color: var(--color-primary); }
