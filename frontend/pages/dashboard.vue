<template>
  <NuxtLayout name="default">
    <div class="space-y-8">
      ...
      <section class="relative overflow-hidden rounded-2xl border border-(--border-color) bg-(--bg-card) p-6 sm:p-8 shadow-(--shadow-sm)">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div class="space-y-1.5 max-w-2xl">
            <Badge variant="primary" :dot="true">{{ authStore.isAdmin ? 'Administrator' : 'Workspace' }}</Badge>
          ...
          <div class="flex items-center gap-3">
            <div class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-(--bg-muted) border border-(--border-color)">
              ...
              <span class="text-xxs font-bold uppercase tracking-widest text-(--text-body) font-mono">Operational</span>
            </div>
      ...
      <!-- Error state -->
      <div v-if="error && !loading" class="rounded-xl border border-(--color-danger)/30 bg-(--color-danger-subtle) px-5 py-4 flex items-center gap-3 text-xs text-(--color-danger)">
        ...
        <section class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div class="glass-card rounded-2xl p-4 space-y-2 col-span-1">
            <div class="flex items-center justify-between">
              <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Employees</span>
            ...
            <p class="text-xxs text-(--text-muted)">
              ...
              <span class="text-(--color-success)">+{{ summary?.kpis?.employees?.new_mtd ?? 0 }} MTD</span>
            </p>
          </div>
          ...
          <div class="glass-card rounded-2xl p-4 space-y-2 col-span-1">
            <div class="flex items-center justify-between">
              <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Revenue MTD</span>
            ...
            <p class="text-xxs" :class="revenueChangeClass">
              <i class="ti text-[10px]" :class="revenueChangePct >= 0 ? 'ti-trending-up' : 'ti-trending-down'" />
          ...
          <div class="glass-card rounded-2xl p-4 space-y-2 col-span-1">
            <div class="flex items-center justify-between">
              <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Orders MTD</span>
          ...
          <div class="glass-card rounded-2xl p-4 space-y-2 col-span-1">
            <div class="flex items-center justify-between">
              <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Customers</span>
          ...
          <div class="glass-card rounded-2xl p-4 space-y-2 col-span-1">
            <div class="flex items-center justify-between">
              <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Pending Leaves</span>
          ...
          <div class="glass-card rounded-2xl p-4 space-y-2 col-span-1">
            <div class="flex items-center justify-between">
              <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Inventory</span>
            ...
            <p class="text-xxs" :class="(summary?.kpis?.inventory?.low_stock ?? 0) > 0 ? 'text-(--color-warning)' : 'text-(--text-muted)'">
              <i v-if="(summary?.kpis?.inventory?.low_stock ?? 0) > 0" class="ti ti-alert-triangle text-[10px]" />
        ...
        <section class="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div class="xl:col-span-2 glass-card rounded-2xl p-6">
            ...
            <div v-else class="mt-5 grid grid-cols-7 gap-2 items-end h-44">
              <div v-for="bar in revenueBars" :key="bar.label" class="flex flex-col items-center gap-1 group" :title="`${bar.label}: ${formatCurrency(bar.amount)}`">
                <div class="w-full bg-(--bg-muted) rounded-md relative overflow-hidden" :style="{ height: `${bar.height}%` }">
                  <div class="absolute inset-0 bg-linear-to-t from-(--color-primary) to-(--color-info) opacity-90 group-hover:opacity-100 transition-opacity" />
                </div>
            ...
            <footer class="mt-5 pt-5 border-t border-(--border-color) grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div>
                ...
                <p class="text-(--text-heading) font-semibold font-mono">{{ formatCurrency(summary?.kpis?.sales?.revenue_mtd ?? 0) }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="text-(--text-heading) font-semibold font-mono">{{ summary?.kpis?.sales?.orders_mtd ?? '—' }}</p>
              </div>
              <div>
                <p class="text-xxs text-(--text-muted) uppercase tracking-widest">Open Leads</p>
                <p class="text-(--text-heading) font-semibold font-mono">{{ summary?.kpis?.sales?.open_leads ?? '—' }}</p>
              </div>
            </footer>
          </div>
          ...
          <div class="space-y-6">
            ...
            <div class="glass-card rounded-2xl p-6">
              ...
              <div v-else-if="headcountBars.length === 0" class="text-xs text-(--text-muted) py-4 text-center">No department data</div>
              <div v-else class="space-y-3">
                <div v-for="bar in headcountBars" :key="bar.label">
                  <div class="flex justify-between text-xs mb-1">
                    <span class="text-(--text-body) truncate flex-1 pr-2">{{ bar.label }}</span>
                    <span class="font-mono font-semibold text-(--text-heading)">{{ bar.count }}</span>
                  </div>
                  <div class="h-2 rounded-full bg-(--bg-muted) overflow-hidden">
                    <div
                      class="h-full rounded-full bg-linear-to-r from-(--color-info) to-(--color-primary) transition-all duration-500"
            ...
            <div class="glass-card rounded-2xl p-5 space-y-3">
              ...
              <div class="grid grid-cols-2 gap-3 text-center">
                <div class="rounded-xl bg-(--bg-muted) py-3">
                  <p class="text-xl font-bold font-mono text-(--color-success)">{{ summary?.kpis?.attendance?.present_today ?? '—' }}</p>
                  <p class="text-xxs text-(--text-muted) mt-0.5">Present</p>
                </div>
                <div class="rounded-xl bg-(--bg-muted) py-3">
                  <p class="text-xl font-bold font-mono" :class="(summary?.kpis?.leave?.pending ?? 0) > 0 ? 'text-(--color-warning)' : 'text-(--text-heading)'">
                    {{ summary?.kpis?.leave?.pending ?? '—' }}
                  </p>
                  <p class="text-xxs text-(--text-muted) mt-0.5">Pending leaves</p>
                </div>
              </div>
              <div v-if="(summary?.kpis?.finance?.unposted_journals ?? 0) > 0" class="flex items-center gap-2 rounded-lg badge-soft-warning px-3 py-2 text-xxs">
                <i class="ti ti-alert-triangle text-sm shrink-0" />
                <span>{{ summary?.kpis?.finance?.unposted_journals }} unposted journal{{ (summary?.kpis?.finance?.unposted_journals ?? 0) !== 1 ? 's' : '' }}</span>
        ...
        <section class="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div class="glass-card rounded-2xl">
            <header class="flex items-center justify-between px-5 py-4 border-b border-(--border-color)">
              ...
              <NuxtLink to="/sales/orders" class="text-xs font-semibold text-(--color-primary) hover:underline">View all →</NuxtLink>
            </header>
            <div v-if="loading" class="p-5 space-y-3">
              <div v-for="i in 5" :key="i" class="h-8 rounded dash-skeleton" />
            </div>
            <div v-else-if="!recentOrders.length" class="px-5 py-8 text-center text-xs text-(--text-muted)">No orders yet</div>
              ...
              <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-(--border-color)">
                  <tr v-for="o in recentOrders" :key="o.id" class="hover:bg-(--bg-muted) transition-colors">
                    <td class="px-5 py-3 text-xs font-mono text-(--text-heading) font-semibold">{{ o.number }}</td>
          ...
          <div class="glass-card rounded-2xl">
            <header class="flex items-center justify-between px-5 py-4 border-b border-(--border-color)">
              ...
              <NuxtLink to="/leaves" class="text-xs font-semibold text-(--color-primary) hover:underline">View all →</NuxtLink>
            </header>
            <div v-if="loading" class="p-5 space-y-3">
              <div v-for="i in 5" :key="i" class="h-8 rounded dash-skeleton" />
            </div>
            <div v-else-if="!recentLeaves.length" class="px-5 py-8 text-center text-xs text-(--text-muted)">No pending requests</div>
            <div v-else class="overflow-x-auto">
              <table class="w-full text-left">
                <thead>
                  <tr class="text-xxs uppercase tracking-wider text-(--text-muted) border-b border-(--border-color)">
                ...
                <tbody class="divide-y divide-(--border-color)">
                  <tr v-for="l in recentLeaves" :key="l.id" class="hover:bg-(--bg-muted) transition-colors">
                    <td class="px-5 py-3 text-xs font-semibold text-(--text-heading)">{{ l.employee_name || '—' }}</td>
                    <td class="px-5 py-3 text-xs">{{ l.type }}</td>
                    <td class="px-5 py-3 text-xs text-right font-mono font-semibold text-(--text-heading)">{{ l.days }}d</td>
      ...
      <template v-else>
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <template v-if="hasModule('hrm')">
            <div class="glass-card rounded-2xl p-5 space-y-2">
              <div class="flex items-center justify-between">
                <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">My Team</span>
                <span class="w-7 h-7 rounded-lg badge-soft-primary flex items-center justify-center">
                  <i class="ti ti-users text-sm" />
                </span>
              </div>
              ...
              <p class="text-xxs text-(--text-muted)">{{ summary?.kpis?.employees?.on_leave ?? 0 }} on leave today</p>
            </div>
            <div class="glass-card rounded-2xl p-5 space-y-2">
              <div class="flex items-center justify-between">
                <span class="text-xxs font-bold uppercase tracking-widest text-(--text-muted)">Leave Requests</span>
                <span class="w-7 h-7 rounded-lg badge-soft-warning flex items-center justify-center">
                  <i class="ti ti-calendar-event text-sm" />
                </span>
              </div>
          ...
          <template v-if="hasModule('sales')">
            ...
            <div class="glass-card rounded-2xl p-5 space-y-2">
              ...
              <p class="text-xxs text-(--text-muted)">{{ summary?.kpis?.sales?.open_leads ?? 0 }} open leads</p>
            </div>
          </template>

          <!-- Fallback when no relevant modules active -->
          <div
            v-if="!hasModule('hrm') && !hasModule('sales')"
          ...
          >
            ...
            <p class="text-xs text-(--text-muted) mt-1">Contact your administrator to enable modules for this workspace.</p>
          </div>
        ...
        <div v-if="hasModule('hrm')" class="glass-card rounded-2xl">
          ...
          <div v-else-if="!recentLeaves.length" class="px-5 py-8 text-center text-xs text-(--text-muted)">No pending leave requests</div>
          <div v-else class="overflow-x-auto">
            <table class="w-full text-left">
              <thead>
                <tr class="text-xxs uppercase tracking-wider text-(--text-muted) border-b border-(--border-color)">
