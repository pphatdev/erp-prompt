    ...
    <div v-else class="space-y-6">
      ...
      <section class="glass-card rounded-2xl p-6 md:p-7">
        <div class="flex flex-col md:flex-row items-start md:items-center gap-5">
          ...
          <div class="relative shrink-0">
            <div
              class="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center text-xl md:text-2xl font-bold bg-(--color-primary-subtle) text-(--color-primary)"
              :title="employee.fullName"
            >
              {{ initials(employee) }}
            </div>
            <span
              class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-(--bg-card)"
              :class="statusDotClass(employee.status)"
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-(--text-muted) mt-1 truncate">
              <span class="text-(--text-body)">{{ employee.position?.title || 'Unassigned position' }}</span>
      ...
      <nav class="glass-card rounded-xl px-2 py-1.5 flex items-center gap-1 overflow-x-auto">
        <button
          ...
          type="button"
          class="tab-trigger"
          :class="{ 'tab-trigger-active': activeTab === t.key }"
          @click="setTab(t.key)"
        >
          <i :class="['ti', t.icon]" />
          <span>{{ t.label }}</span>
      ...
      <section v-else-if="activeTab === 'compensation'" class="glass-card rounded-2xl p-5 space-y-5">
        ...
        <p class="text-xxs text-(--text-muted)">
          Salary &amp; bank fields are encrypted at rest and masked client-side. Updates require <code class="font-mono">hrm.employee.write</code>.
        </p>
      </section>
      ...
      <section v-else-if="activeTab === 'attendance'" class="space-y-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <div class="glass-card rounded-xl p-4">
            ...
            <p class="text-lg font-mono font-semibold text-(--color-success) mt-1">{{ attendance.summary.present }}</p>
          </div>
          <div class="glass-card rounded-xl p-4">
            <p class="text-xxs uppercase tracking-widest font-bold text-(--text-muted)">Late</p>
            <p class="text-lg font-mono font-semibold text-(--color-warning) mt-1">{{ attendance.summary.late }}</p>
          </div>
          <div class="glass-card rounded-xl p-4">
            <p class="text-xxs uppercase tracking-widest font-bold text-(--text-muted)">Half day</p>
            <p class="text-lg font-mono font-semibold text-(--color-warning) mt-1">{{ attendance.summary.halfDay }}</p>
          </div>
          <div class="glass-card rounded-xl p-4">
            <p class="text-xxs uppercase tracking-widest font-bold text-(--text-muted)">Absent</p>
            <p class="text-lg font-mono font-semibold text-(--color-danger) mt-1">{{ attendance.summary.absent }}</p>
          </div>
          <div class="glass-card rounded-xl p-4">
            <p class="text-xxs uppercase tracking-widest font-bold text-(--text-muted)">Paid leave</p>
            <p class="text-lg font-mono font-semibold text-(--color-info, --color-primary) mt-1">{{ attendance.summary.paidLeave }}</p>
          </div>
          <div class="glass-card rounded-xl p-4">
            <p class="text-xxs uppercase tracking-widest font-bold text-(--text-muted)">Unpaid leave</p>
            <p class="text-lg font-mono font-semibold text-(--color-danger) mt-1">{{ attendance.summary.unpaidLeave }}</p>
          </div>
        </div>
        ...
        <div class="glass-card rounded-2xl overflow-hidden">
          <header class="flex items-center justify-between px-5 py-3 border-b border-(--border-color)">
            ...
            <span class="text-xxs text-(--text-muted) font-mono">{{ attendance.list.length }} row{{ attendance.list.length === 1 ? '' : 's' }}</span>
          </header>
          ...
          <table v-else class="w-full text-left">
            ...
            <tbody class="divide-y divide-(--border-color)">
              <tr v-for="a in attendance.list" :key="a.id" class="hover:bg-(--bg-muted) transition-colors">
                <td class="px-5 py-2.5 font-mono text-xs">{{ a.date }}</td>
                <td class="px-5 py-2.5 font-mono text-xxs text-(--text-muted)">{{ formatAttendanceTime(a.checkIn) }}</td>
                <td class="px-5 py-2.5 font-mono text-xxs text-(--text-muted)">{{ formatAttendanceTime(a.checkOut) }}</td>
                <td class="px-5 py-2.5">
                  <Badge :variant="attendanceStatusVariant(a.status)" :dot="true">{{ a.status.replace('_', ' ') }}</Badge>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <section v-else-if="activeTab === 'leaves'" class="space-y-4">
        ...
        <div v-else-if="leaves.balance.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          >
            ...
            <p class="text-lg font-mono font-semibold text-(--color-primary) mt-1">
              {{ b.remaining }} <span class="text-xxs text-(--text-muted) font-normal">/ {{ b.annualAllowance }} d</span>
            </p>
            <p class="text-xxs text-(--text-muted) mt-1">{{ b.used }} day{{ b.used === 1 ? '' : 's' }} used</p>
          </div>
        </div>
        ...
        <div class="glass-card rounded-2xl overflow-hidden">
          <header class="flex items-center justify-between px-5 py-3 border-b border-(--border-color)">
            ...
            <span class="text-xxs text-(--text-muted) font-mono">{{ leaves.list.length }} record{{ leaves.list.length === 1 ? '' : 's' }}</span>
          </header>
          ...
          <table v-else class="w-full text-left">
            ...
            <tbody class="divide-y divide-(--border-color)">
              <tr v-for="lv in leaves.list" :key="lv.id" class="hover:bg-(--bg-muted) transition-colors">
                ...
                <td class="px-5 py-2.5 text-xs text-(--text-body) max-w-[260px] truncate" :title="lv.reason || ''">
                  {{ lv.reason || '—' }}
                </td>
                <td class="px-5 py-2.5">
      ...
      <section v-else-if="activeTab === 'payslips'" class="glass-card rounded-2xl overflow-hidden">
        <header class="flex items-center justify-between px-5 py-3 border-b border-(--border-color)">
          ...
          <span class="text-xxs text-(--text-muted) font-mono">{{ payslips.list.length }} record{{ payslips.list.length === 1 ? '' : 's' }}</span>
        </header>
          ...
          <tbody class="divide-y divide-(--border-color)">
            <tr v-for="ps in payslips.list" :key="ps.id" class="hover:bg-(--bg-muted) transition-colors">
              <td class="px-5 py-2.5 text-xs">
                <div class="font-semibold text-(--text-heading)">{{ ps.payrollPeriod?.name || '—' }}</div>
      ...
      <section v-else-if="activeTab === 'appraisals'" class="glass-card rounded-2xl overflow-hidden">
        <header class="flex items-center justify-between px-5 py-3 border-b border-(--border-color)">
          ...
          <span class="text-xxs text-(--text-muted) font-mono">{{ appraisals.list.length }} record{{ appraisals.list.length === 1 ? '' : 's' }}</span>
        </header>
...
.icon-btn:hover { background: var(--color-primary-subtle); color: var(--color-primary); }
...
.tab-trigger:hover { color: var(--text-heading); background: var(--bg-muted); }
.tab-trigger-active {
