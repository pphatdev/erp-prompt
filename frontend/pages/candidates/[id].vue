    ...
    <div v-else class="space-y-6">
      ...
      <section class="glass-card rounded-2xl p-6 md:p-7">
        <div class="flex flex-col md:flex-row items-start md:items-center gap-5">
          <!-- Avatar -->
          <div
            class="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center text-xl md:text-2xl font-bold bg-(--color-primary-subtle) text-(--color-primary) shrink-0"
      ...
      <section class="flex flex-wrap items-center gap-3 relative">
        <div class="relative">
          <button
            ...
            :disabled="!nextStages.length || advancing"
            @click="openAdvanceMenu = !openAdvanceMenu"
          >
          ...
          <div
            v-if="openAdvanceMenu && nextStages.length"
        ...
        <div class="lg:col-span-8 space-y-6">
          ...
          <article class="glass-card rounded-2xl p-6 space-y-4">
            ...
            <p
              ...
              class="text-xs text-(--text-body) leading-relaxed whitespace-pre-wrap"
            >{{ app.coverLetter }}</p>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 pt-3 border-t border-(--border-color)">
              <div class="stat-tile">
                <p class="stat-label"><i class="ti ti-star-filled text-[12px] text-(--color-warning)" /> Rating</p>
                <p class="stat-value text-(--color-warning)">{{ rating.toFixed(1) }}</p>
              </div>
              <div class="stat-tile">
                <p class="stat-label"><i class="ti ti-calendar-time text-[12px] text-(--color-info)" /> Days in pipeline</p>
                <p class="stat-value text-(--color-info)">{{ daysInPipeline }}</p>
              </div>
              <div v-if="canSeeSalary" class="stat-tile">
                <p class="stat-label"><i class="ti ti-coin text-[12px] text-(--color-primary)" /> Expected</p>
                <p class="stat-value text-(--color-primary) font-mono">
                  {{ app.expectedSalary != null ? formatMoney(app.expectedSalary) : '—' }}
                </p>
              </div>
              <div class="stat-tile">
                <p class="stat-label"><i class="ti ti-route text-[12px] text-(--color-secondary)" /> Source</p>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <article class="glass-card rounded-2xl p-6 space-y-4">
              ...
              <div>
                <p class="text-xxs uppercase tracking-widest font-bold text-(--text-muted) mb-2">Target vacancy</p>
                <NuxtLink
                  v-if="app.vacancy"
                  :to="`/candidates?vacancyId=${app.vacancy.id}`"
                ...
                >
                  <span class="w-10 h-10 rounded-lg bg-(--color-primary-subtle) text-(--color-primary) inline-flex items-center justify-center shrink-0">
                    <i class="ti ti-briefcase text-lg" />
                  </span>
                  <div class="min-w-0">
                    <p class="text-xs font-semibold text-(--text-heading) truncate">{{ app.vacancy.title }}</p>
              ...
              <div>
                <p class="text-xxs uppercase tracking-widest font-bold text-(--text-muted) mb-2">Referred by</p>
                <div
                  v-if="referrer"
                  class="flex items-center gap-3 p-3 rounded-lg bg-(--bg-muted) border border-(--border-color)"
                >
                  <span class="w-9 h-9 rounded-full bg-(--color-secondary-subtle) text-(--color-secondary) inline-flex items-center justify-center font-bold text-xxs shrink-0">
                    {{ initials(referrer.fullName) }}
                  </span>
                  <div class="min-w-0">
                    <p class="text-xs font-semibold text-(--text-heading) truncate">{{ referrer.fullName }}</p>
                    <p class="text-xxs text-(--text-muted) font-mono">{{ referrer.employeeId }}</p>
                  </div>
                </div>
                <p v-else class="text-xxs text-(--text-muted) italic">No referrer — direct application.</p>
              </div>
            </article>
            ...
            <article class="glass-card rounded-2xl p-6 space-y-3">
              ...
              >
                ...
                <span class="flex-1 min-w-0">
                  <span class="block text-xs font-semibold text-(--text-heading) truncate">Resume / CV</span>
                  <span class="block text-xxs text-(--text-muted) font-mono truncate">{{ app.resumePath }}</span>
                </span>
              ...
              >
                ...
                <span class="flex-1 min-w-0">
                  <span class="block text-xs font-semibold text-(--text-heading)">LinkedIn profile</span>
                  <span class="block text-xxs text-(--text-muted) font-mono truncate">{{ app.linkedinUrl }}</span>
                </span>
          ...
          <article class="glass-card rounded-2xl p-6 space-y-4">
            ...
            <div v-if="!workExperience.length" class="rounded-lg border border-dashed border-(--border-color) text-xxs text-(--text-muted) py-6 text-center">
            ...
            <ol v-else class="relative border-l-2 border-(--border-color) ml-2 space-y-5 pl-5">
              <li v-for="(exp, idx) in workExperience" :key="idx" class="relative">
                <span class="absolute -left-[26px] top-1 w-4 h-4 rounded-full bg-(--color-primary) border-2 border-(--bg-card)" />
                ...
                <p class="text-xxs text-(--color-primary) font-semibold mt-0.5">
                  {{ exp.company || '—' }}
                  <span v-if="exp.location" class="text-(--text-muted) font-normal"> · {{ exp.location }}</span>
                </p>
                <p v-if="exp.description" class="text-xxs text-(--text-body) mt-2 leading-relaxed whitespace-pre-line">
                  {{ exp.description }}
                </p>
              </li>
            </ol>
        ...
        <div class="lg:col-span-4 space-y-6">
          ...
          <article class="glass-card rounded-2xl p-6 space-y-4">
            ...
            <ul v-else class="space-y-3">
              ...
              >
                <span class="w-8 h-8 rounded-lg bg-(--color-info-subtle) text-(--color-info) inline-flex items-center justify-center shrink-0">
          ...
          <article class="glass-card rounded-2xl p-6 space-y-4">
            ...
            <ol class="relative space-y-4 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-(--border-color)">
              <li
          ...
          >
            ...
            <NuxtLink
              v-if="app.employeeId"
              :to="`/employees?id=${app.employeeId}`"
