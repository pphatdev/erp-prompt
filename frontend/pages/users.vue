    ...
    <div class="space-y-6">
      ...
      <section class="glass-card rounded-xl p-4 flex flex-col md:flex-row gap-3 items-center justify-between">
        <div class="relative w-full md:w-80">
          ...
          <input v-model="searchQuery" type="search" placeholder="Search by name or email..." class="form-control pl-9" />
        </div>
      ...
      <section v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <article v-for="user in filteredUsers" :key="user.id" class="glass-card rounded-2xl p-5 flex flex-col gap-4 group">
          <header class="flex items-start justify-between">
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 rounded-lg bg-(--color-primary-subtle) text-(--color-primary) flex items-center justify-center font-semibold">
                {{ user.name.charAt(0).toUpperCase() }}
              </div>
              <div class="min-w-0">
                <h3 class="text-sm font-semibold text-(--text-heading) group-hover:text-(--color-primary) transition-colors truncate">{{ user.name }}</h3>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="glass-card rounded-2xl w-full max-w-md p-6 shadow-(--shadow-lg) bg-(--bg-card)">
          ...
          <form @submit.prevent="saveUser" class="space-y-4">
            <div>
              <label class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Full name</label>
              <input v-model="form.name" type="text" required class="form-control" />
            </div>
            <div>
              <label class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Email address</label>
              <input v-model="form.email" type="email" required class="form-control" />
            </div>
            <div v-if="!editingUser">
              <label class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Security password</label>
              <input v-model="form.password" type="password" required class="form-control" />
            </div>

            <div>
              <label class="block text-xxs font-bold text-(--text-muted) uppercase tracking-wide mb-1.5">Bind authority roles</label>
              <div class="space-y-1 max-h-32 overflow-y-auto p-2 bg-(--bg-muted) border border-(--border-color) rounded-lg custom-scrollbar">
                <label v-for="role in rolesList" :key="role.id" class="flex items-center gap-2 px-1.5 py-1 rounded hover:bg-(--bg-card) cursor-pointer">
                  <input
                    type="checkbox"
                    :value="role.id"
            ...
            <label v-if="editingUser" class="flex items-center gap-2 py-2 text-xs text-(--text-heading)">
              <input v-model="form.is_active" type="checkbox" class="w-4 h-4 rounded border-(--border-color) text-(--color-primary)" />
              Profile active
            </label>
