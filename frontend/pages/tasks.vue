    ...
    <div class="space-y-6">
      ...
      <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p class="text-xs text-(--text-muted) mt-1">Premium GPU-accelerated visualisations — design.md §9.</p>
        </div>
      ...
      <section class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MeteorCard v-for="sprint in sprints" :key="sprint.title" :variant="sprint.variant">
          <div class="flex items-start justify-between gap-3 relative z-10">
            <div>
              <p class="text-xxs font-bold uppercase tracking-widest" :class="toneText[sprint.variant]">{{ sprint.tag }}</p>
          ...
          <footer class="mt-4 pt-3 border-t border-(--border-color) flex items-center justify-between text-xxs text-(--text-muted) relative z-10">
      ...
      <section class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 glass-card rounded-2xl">
          ...
          <ul class="divide-y divide-(--border-color)">
            <li v-for="t in tasks" :key="t.id" class="px-5 py-4 hover:bg-(--bg-muted)/60 transition-colors flex items-center gap-4">
              <RippleIndicator :variant="t.priority === 'high' ? 'danger' : t.priority === 'medium' ? 'warning' : 'success'" />
              ...
              <div class="hidden sm:flex w-28 bg-(--bg-muted) rounded-full overflow-hidden h-2">
                <div class="h-full bg-linear-to-r from-(--color-primary) to-(--color-info)" :style="{ width: `${t.progress}%` }" />
              </div>
            </li>
          </ul>
        </div>
        ...
        <div class="space-y-4">
          <div v-for="col in board" :key="col.title" class="glass-card rounded-2xl p-5">
            ...
            <ul class="space-y-2">
              <li v-for="i in col.items" :key="i.title" class="p-3 rounded-lg bg-(--bg-muted)/60 border border-(--border-color)">
