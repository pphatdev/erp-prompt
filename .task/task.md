...
- [x] Digital Offer & Onboarding Pipeline (Phase 8: Backend + Frontend shipped — /hrm/offers, /hrm/onboarding, Candidate Profile Offer tab. Phase 8.5 restructure moved Employee creation to Appointment Request approval, so offer.markAccepted only flips status now.)
- [x] Recruitment Candidate Pipeline Settings (Phase 11: Dynamic Kanban stages shipped — WorkflowStatusService gained `enforceSingleInitial` / `assertDeletable` / `slugFromLabel` / `reorderModule` / `setAsInitial`; new endpoints `PATCH /workflow-statuses/reorder` + `POST /workflow-statuses/{id}/set-default`; Store FormRequest auto-slugs `key` from `label`; destroy gated on REFERENCE_MAP lookup. Pest `CandidatePipelineSettingsTest` covers 14 cases. Frontend: WorkflowStatusesSection ships drag-to-reorder grid + Set-as-default action + Kanban metadata form (visibility / sort / display fields / conversion eligible). Candidate Kanban honours stage metadata for column visibility, per-column sort, and per-card display fields.)
- [x] Leave & Time Off Settings (Phase 12: leave_types extended with `code` + `is_paid` + `is_accrued`; new `employee_leave_allocations` ledger; `LeaveAllocationService` with provision / hold / release / apply / cancel / adjust; `LeaveService` consumes the ledger end-to-end (balance, submit, sync, withdraw) with pro-rata cap on accrued types; `WorkScheduleService::countWorkingDaysDecimal` sums per-day interval minutes; `EmployeeCreated` event + `ProvisionLeaveAllocations` listener auto-seed allocations on hire (manual + recruitment paths); `EmployeeLeaveAllocationController` + Pest `LeaveAllocationsTest`; Frontend ships Leave Types extension + `/hrm/timeoff/allocations` master/detail page + live remaining balance + duration preview on the eApprovals Leave form.)
